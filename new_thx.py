...
for target in [1+12*(i//2) + (5 if i%2==0 else 12) for i in range(-2,19)]:
    voice_targets.append(target-random.random() * 0.10)
...
def simulate(voices):
    ...
    for t in range(1, wander_time * rate):
        ...
        for j in range(voices.shape[0]):
            ...
            voice_out = amplitude * math.sin(phase[j]) * (1-math.log(new_freq*5-20)/5/math.log(10))
    ...
    for t in range(wander_time * rate, crescendo_max):
        ...
        for j in range(voices.shape[0]):
            ...
            phase[j] += 2 * math.pi * new_freq / rate
            phase[j] %= 2 * math.pi
            sample += amplitude * math.sin(phase[j]) * (1-math.log(new_freq*5-20)/5/math.log(10))
        output[t] = sample
